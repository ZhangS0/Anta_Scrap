...
- `查询/导出失败: ... /api/page/... -> 500: error_code=1004 无权访问/无授权权限` → BI 服务端该账号无此报表页面权限（非模板/字段问题；权限按 账号×报表 分配）。处置：①查 `docs/ACCOUNT_PERMISSIONS.md` 换有权限账号；②跑 `python scripts/collect_user_pages.py <工号>` 确认该账号可见页面；③必要时联系 BI 管理员开权限
- **查询成功但区域行少 / 区域筛选返回空表头** → 账号**行级数据权限**受限（如仅华东区；2026-09-03 实测 2024071793 查迪桑特日报区域仅返回华东 1 行、筛华北返回空表头）。与 1004 页面权限不同：查询不报错但数据**静默缩减**，极易当全国数据交付。处置：采集前先 `rows [区域]` 查一次核对区域覆盖，见 `docs/ACCOUNT_PERMISSIONS.md`「行级数据权限」
